...
function get_module_settting($shop_id,$module_key)
{
    global $config;
    $ref = 0;
    $module_upgrades_info = ORM::for_table($config['db']['pre'].'module_upgrades')
    ->where(array('shop_id'=>$shop_id, 'module_id' => $module_key))
    ->find_one();
  if(isset($module_upgrades_info['id']))
  {
  $active = $module_upgrades_info['active'];
  $upgrade_expires = date("Y-m-d",$module_upgrades_info['upgrade_expires']);
  $date_now = date("Y-m-d");
  if($active == "1" && $upgrade_expires >= $date_now)
  {
      $ref = 1;
  }
  else
...
function get_all_setting($user_id)
{
    global $config;
    // Get usergroup details
 $group_id = get_user_group_by_id($user_id);
...
function get_allow_order($user_id)
{
    global $config;
    // Get usergroup details
 $group_id = get_user_group_by_id($user_id);
...
function check_customers_account_exists($email,$restaurant_id){
...
function check_customers_username_exists($username,$restaurant_id){
...
function create_user_session($userid,$username,$password,$user_type = '', $shop_id, $full_name=''){
...
function userlogin($email,$password)
{
    ...
    if ($num_rows >= 1) {
        $info = ORM::for_table($config['db']['pre'].'user')
            ->select_many('id', 'status', 'username', 'password_hash', 'user_type', 'name', 'shop_id')
            ->where($where, $email)
            ->find_one();
...
function get_user_data($username=null,$userid=null){
    ...
    if (isset($info['id'])) {
        ...
        $userinfo['firebase_token']    = $info['firebase_token'];
        return $userinfo;
    }
    else
    {
...
function isEmployer(){
    return isset($_SESSION['user']['user_type']) ? $_SESSION['user']['user_type'] == 'employer' : 0;
}
?>
