...
$config['email_template'] = 0;
$config['email_result'] = array('error' => 0, 'message' => '');
...
if($config['email_type'] == 'smtp'){
    ...
    $mail->SetFrom($config['smtp_username'], $name = "hallo");    
    ...
    /* Send Error */
    if(!$mail->Send()){        
        $config['email_result']['error'] = 1;
        $config['email_result']['message'] = $mail->ErrorInfo;
        return false;
        //echo $mail->ErrorInfo;
    }else{
        return true;
...
# PHPMail*******************************************************************************
    ...
    ///$mail->SetFrom($config['admin_email'], $name = $config['site_title']);
    $name_send = isset($name) ? $name : $config['site_title'];
    $mail->SetFrom($config['smtp_username'], $name_send);    
    if($email_reply_to != null){
        $mail->AddReplyTo($email_reply_to, $email_reply_to_name);
    }
