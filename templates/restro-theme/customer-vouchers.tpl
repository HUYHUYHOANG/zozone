                ...
                <div class="col-xl-12">
                    <div class="margin-top-0">                      
                        <div class="right-toolbox menu-button">
                            ...
                            <a href="./customers?d0=customer-vouchers&id={THE_CUSTOMER_ID}" class="button disabled margin-left-auto">{LANG_VOUCHERS} <i class="icon-feather-tag"></i></a>
...
<script type="text/javascript">
    boConfigParams = $.extend({selectedStatus : [], ajxAction : 'customers', defaultView : 'load-vouchers'}, boConfigParams);    
    ...
    $.initListview = function(){
        $('.vouchers-table .delete-voucher').click(function(){
            var theID = $(this).data('id');
            $.deleteRecd($(this), theID);
        });

        $('.vouchers-table .qr-code').click(function(){
            var theID = $(this).data('id');
            var _this = $(this);
            boDialog.wrapper.mfpPopup(boDialog, function(){                
                var qr_link = '{SITE_URL}/vouchers?d0=use-voucher&code=' + _this.data('code');
                var url = boConfigParams.boSvrSide + '&m=vouchers&d0=generate-qr-code&code=' + qr_link.encode();
                boDialog.wrapper.find('a.tab-title').text(_this.data('code'));
                boDialog.contentWrapper.append('<div class="qrcode-wrap"><img src="' + url +'"/></div>').removeClass('loading');
            });
        });
    };

    $.deleteRecd = function(btn, theID){
