...
class CServiceBookingV20{    
    ...
    public function add2cart(){
        ...
        if(isset($json->$langCode->title)) $name = $json->$langCode->title;
        else $name = $item->name;
    ...
    private function checkTimeSlot($atimes, $slot, $totalDuration){
        ...
        foreach($atimes as $time){
            ...
            if($slot1>=$start && $slot1<=$end){
                return ($time['status']==1 || $time['status']==3);
            }
            //continue;
            if($totalDuration){
    ...
    public function addBooking(&$customer, &$bookingData, &$returnObj, &$theCustomerObj){
        ...
        if(!$error){
            ...
            if($id = $insertData->id()){
                ...
                //send booking email to shop
                $shop = ORM::for_table($this->tblPre.'shop')->where('id', $this->shopId)->find_one(); 
                $subject = "New reservations";
                $staus = "Bestätigt";// xác nhận
                $theCustomerObj->sendBookingEmail($customer,$insertData,$this->cart->items,$shop['email'],$shop['name'],$subject,$staus);
                // //send booking email to customer    
                $theCustomerObj->sendBookingEmail($customer,$insertData,$this->cart->items,$customer->email,$customer->name,$subject,$staus);
                if($payment=='paypal'){
                    $this->prepareDataForOnlinePayment($id, $amt, $customer, $voucher, $payment, $returnObj);
                }
                
                if ($this->getShopOtion('quickorder_enable', 0)) {
                    $whatsapp_number = $this->getShopOtion('whatsapp_number');      
                    $whatsapp_message = "*Buchungsdetails*" . "\n";
                    $today = date("d-m-Y H:i:s");
                    $serviceDate = date("d.m.Y H:i:s", strtotime($date));

                    $whatsapp_message .= "Datum: " . $serviceDate . "\n";
                    $rows = '';
                    $totalDuration = 0; $subAmt = 0; $idx = 1;          
                    foreach($this->cart->items as $item){
                        $itemData = ORM::for_table($this->tblPre.'menu')
                        ->where('id', $item['id'])
                        ->find_one();
                        $subAmt += $item['price'];
                        $totalDuration += $item['duration'];        
                        $itemPrice = number_format($item['price'], 2, ',', '.');
                        $row = "▪️ " .  $itemData['name'] . ' - ' . $item['duration'] . ' Minute : ' . $itemPrice  . "\n";
                        $rows .= $row;
                        ++$idx;
                    }
                    ...
                    $footer .= "*Gesamtbetrag: " . $str  . ' ( ' . $totalDuration . ' Minute)*';
                    $whatsapp_message .= $footer . "\n"; 
                    
                    $whatsapp_message .= "*Angaben zum Kunden*" . "\n";

                    $icon_phone = "☎️";
                    $icon_address = "📌";
                    $icon_email = "📧";

                    $whatsapp_message .= $customer->name . "\n";
                    $whatsapp_message .= $icon_phone . $customer->phone . "\n";
                    $whatsapp_message .= $icon_email . $customer->email . "\n";
                    $whatsapp_message .= "-----------------------------" . "\n";
                    $whatsapp_message .= "Vielen Dank für Ihre Reservierung. Wir möchten Ihnen hiermit bestätigen, dass Ihre Reservierung erfolgreich abgeschlossen wurde. Wir freuen uns darauf, Sie bei uns begrüßen zu dürfen!" . "\n";

                   $returnObj->whatsApp_url  = 'https://api.whatsapp.com/send?phone=' . $whatsapp_number . '&text=' . urlencode($whatsapp_message);                          
                }
                 //clean up cart
                 $this->cart->clear();
            }else{
    ...
    private function buildCartContent($customer, $voucher, $payment, $amount, $reducedAmount, $date){
        ...
        if($voucher && $reducedAmount){
            ...
            $footer = "<tr><td></td><td colspan='3'>{$str}</td></tr>";
            $str = number_format($reducedAmount, 2, '.', ',');
            $footer .= "<tr><td></td><td>" . $lang['VOUCHER'] . " : {$voucher}</td><td>- {$str}</td></tr>";
        }

        $str = number_format($totalAmount, 2, '.', ',');
        $footer .= "<tr><td></td><td>" . $lang['TOTAL'] . "</td><td>{$totalDuration}</td><td>{$str}</td></tr>";
