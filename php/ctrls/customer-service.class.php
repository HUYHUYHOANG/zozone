...
class CCustomerService{    
    ...
    public function getCustmerData($email=null, $userid=null){
        if($email != null)
            $info = ORM::for_table($this->tableName)->where(array('email'=>$email,'shop_id' => $this->shopId))->find_one();
        else
    ...
    public function addNewCustomer(&$custData){
        ...
        else
        {
            ...
            $sql = "UPDATE ". $this->tableName ." SET name=:name,phone=:phone,last_activity=:activity, newsletter =:newslettter,password_hash=:password_hash  WHERE email=:email and shop_id =:shop_id";
            $success = ORM::get_db()->prepare($sql)->execute($data);
            if($success){
                $data = ORM::for_table($this->tableName)->where('email', $email)->where('shop_id',$this->shopId)->find_one();
                $this->data = $data;
                return 1;
            }
    ...
    public function completeCustomerBooking(){
        ...
        if(!$error){
            ...
            $response = $thebook->addBooking($this->data, $_POST, $this->returnError, $this);    
            $errorType = 'booking';
        }   
    
        $this->returnError->errorType = $errorType;  
        die(json_encode($this->returnError));
    }
    ...
    function sendBookingEmail($customer,$reservations,$items,$email_to,$email_name, $subject, $status)
{
    global $config, $lang, $link;
    CShopInfo::getInfo($this->shopId);
    ...
    $page->SetParameter('SHOP_TELEPHONE', $shop_telephone);
    ...
    $page->SetParameter('CUSTOMER_NAME', $customer->name);
    $page->SetParameter('CUSTOMER_TELEPHONE', $customer->phone);
    $page->SetParameter('CUSTOMER_EMAIL', $customer->email);
    $page->SetParameter('TOTAL_SUM', $totalAmount);
    $page->SetParameter('RESERVATIONS_STATUS_CODE', 0); // display cancel link
    ...
    $this->email($email_to,$email_name , $subject , $email_body);
