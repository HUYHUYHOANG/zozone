...
if(checkloggedin())
{
    ...
    if(isset($_POST['submit']))
    {
        ...
        if(!empty($_POST["password"]))
        {
            ...
            if (strlen($_POST["password"]) < 8 OR (strlen($_POST['password']) > 20)) {
                $errors++;
                $password_error = $lang['PLEASE_ENTER_A_PASSWORD_WITH_A_LENGTH_OF_8_20_CHARACTERS'];
                $password_error = "<span class='status-not-available'> ".$password_error.".</span>";
          ...
          // $person->set('menu_layout', $_POST["menu_layout"]);
           ...
           //  die();
            ...
            if(!empty($_POST['menu_languages']) && is_array($_POST['menu_languages'])){
                update_user_option($_SESSION['user']['id'],'shop_menu_languages',implode(',',$_POST['menu_languages']));
            }else{
                update_user_option($_SESSION['user']['id'],'shop_menu_languages','');
            }
            //Updating Session Values
            $loggedin = get_user_data("",$_SESSION['user']['id']);
            create_user_session($loggedin['id'],$loggedin['username'],$loggedin['password'],'',$loggedin['shop_id']);
    ...
    ->find_one();
    $setting = get_all_setting($_SESSION['user']['id']);
    $RESTAURANT_MANAGER_ACCOUNT = 1;//$setting['allow_manager_account'];
    if ($RESTAURANT_MANAGER_ACCOUNT == 1)
    {
        ...
        $menu_languages = get_user_option($_SESSION['user']['id'],'shop_menu_languages','');
