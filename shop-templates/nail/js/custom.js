...
var BeautyZone = function(){
	...
	var onePageLayout = function() {
		...
		if(jQuery('.navbar-nav-scroll').length > 0){
			...
			//smoothscroll
			jQuery('.navbar-nav-scroll a[href^="#"], a.tp-caption[href^="#"]').on('click', function (e) {				
				e.preventDefault();
	...
	var pageOnScroll = function(event){
		...
		jQuery('.navbar-nav-scroll > li').each(function () {
			try{
				...
				if(elementLink.attr("href")=='#booking') return;
				var headerFullHeight =   parseInt($('.main-bar').css('height'), 10);
	...
	var dzTheme = function(){
		...
		if(screenWidth < 991)
		{
			if($('.mo-left .header-nav').children('div').length == 0){
				...
				jQuery('.mo-left .header-nav .logo-header > a > img').attr('src', jQuery('.mo-left .header-nav .logo-header > a > img').data('src'));
	...
	var smartWizard = function (){
		if(($('#smartwizard').length > 0)){
			...
			$('#smartwizard').smartWizard({
				...
				lang : {
					next : zozoneNailParams.lang.nextBtnText,
					previous : zozoneNailParams.lang.prevBtnText
				},
				tabContentShownCallback : $zozoneTabContentShownCallback
			});
	...
	var datetimepicker = function (){
		...
		if(($('#datetimepicker').length > 0)){			
			...
			$('#datetimepicker').datetimepicker({
				format: 'MM/DD/YYYY', minDate : today,
				sideBySide: true,
				widgetPositioning: {
					horizontal: "left",
            		vertical: "bottom"
